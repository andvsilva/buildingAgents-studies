...
and adapting in dynamic environments. This article provides a complete \colorbox{yellow}{introduction to AI agents, covering their core concepts, 
architectures,} \colorbox{yellow}{types, applications, and challenges} \cite{anthropic2024}.
...
\colorbox{green!20}{Modern AI agents are often powered by Large Language Models (LLMs) and extended} 
\colorbox{green!20}{with tools, memory, and feedback loops.}
...
\colorbox{yellow}{Every agent operates with a clear objective, such as answering a question, analyzing data, }
\colorbox{yellow}{or completing a workflow. Without a well-defined goal, effective reasoning is not possible.}
...
\begin{center}
\colorbox{red!15}{\texttt{Goal $\rightarrow$ Plan $\rightarrow$ Act $\rightarrow$ Observe $\rightarrow$ Evaluate $\rightarrow$ Adjust}}
\end{center}
...
\colorbox{green!20}{Multi-agent systems consist of multiple agents working collaboratively or competitively.} 
\colorbox{green!20}{Each agent typically has a specialized role, increasing scalability and robustness.}

\section{Multi-Agent Systems}

In a multi-agent system, responsibilities are distributed among agents. Typical roles include:
\begin{itemize}
  \item \colorbox{green!20}{Planner Agent:} task decomposition and strategy definition.
  \item \colorbox{green!20}{Executor Agent:} performs actions and tool calls.
  \item \colorbox{green!20}{Data Agent:} handles data ingestion and transformation.
  \item \colorbox{green!20}{Analyst Agent:} performs statistical or analytical tasks.
  \item \colorbox{green!20}{Critic Agent:} validates outputs and detects errors.
\end{itemize}
